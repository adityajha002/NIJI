...
const toFormValues = (shop: Partial<ApiShop> = {}): EditShopFormData => ({
  ...
  tags: typeof shop.tags === 'string' ? shop.tags : Array.isArray(shop.tags) ? shop.tags.join(', ') : "",
...
export default function EditShop() {
  ...
  useEffect(() => {
    const fetchShop = async () => {
      ...
      try {
        const result: Partial<ApiShop> = await fetchShopDashboard(token);
  ...
  const onSubmit = async (data: EditShopFormData) => {
    ...
    try {
      const result = await updateShopDashboard(
        {
          name: data.shopName,
