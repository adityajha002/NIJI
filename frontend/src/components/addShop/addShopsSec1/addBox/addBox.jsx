...
const AddBox = () => {
  ...
  const handleGetLocation = async () => {
    ...
    try {
      ...
      setLocationError(
        err?.message === 'Geolocation is not supported'
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Success: clear the form, the image preview, and the captured location
      reset();
      setCoords({ latitude: null, longitude: null });
      ...
      <div className={style.gridbox}>
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          <div className={style.imagesec}>
            ...
            />
            {errors.image && <p className={style.fieldError}>{errors.image.message}</p>}
          </div>
          ...
          {errors.category && <p className={style.fieldError}>{errors.category.message}</p>}
          ...
          <button
            ...
            disabled={locating}
          >
            {locating ? 'LOCATING…' : coords.latitude ? 'LOCATION CAPTURED ✓' : 'GET LOCATION'}
          </button>
          {locationError && <p className={style.fieldError}>{locationError}</p>}
          ...
          />
          {errors.address && <p className={style.fieldError}>{errors.address.message}</p>}
          ...
          />
          {errors.pincode && <p className={style.fieldError}>{errors.pincode.message}</p>}

          {submitError && <p className={style.formError}>{submitError}</p>}
          {submitSuccess && <p className={style.formSuccess}>Shop created successfully!</p>}

          <button className={style.addbtn} type="submit" disabled={isSubmitting}>
